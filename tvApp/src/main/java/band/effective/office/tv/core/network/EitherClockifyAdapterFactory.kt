...
class EitherClockifyAdapterFactory @Inject constructor(): CallAdapter.Factory() {
    override fun get(
        returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<ClockifyApiError>(
            ClockifyApiError::class.java,
            annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
        ...
        private val errorBodyConverter: Converter<ResponseBody, ClockifyApiError>
    ) :
        ...
        private fun wrapResponse(response: Response<T>): Response<Either<ErrorReason, F>> {
            return try {
                if (response.isSuccessful) {
                    ...
                    val reason = ErrorReason.ServerError(errorResponse.message)
