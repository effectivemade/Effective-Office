        ...
        workTogetherUseCase.getNotionDataForStories()
            .flatMapConcat { notionUsers ->
                when (notionUsers) {
                    is Either.Success -> {
                        combine(
                            ...
                            when (duolingoData) {
                                ...
                                is Either.Failure -> error = duolingoData.error
                            }
                            when (clockifyData) {
                                is Either.Success -> storyModels += clockifyData.data
                                is Either.Failure -> error = clockifyData.error
                            }
                            when (supernovaData) {
                    ...
                    is Either.Failure -> return@flatMapConcat flowOf(
                        Either.Failure(
                        "Error in EventStoryDataCombinerUseCase workTogetherUseCase.getNotionDataForStories()"
                        )
                    )
                }
            }
